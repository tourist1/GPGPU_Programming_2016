...
int main(int argc, char **argv)
{
	...
	bool sucb, suct, sucm; // check if file is successfully opened
	int wb, hb, cb, wt, ht, ct, wm, hm, cm; // W, H, C of background, target and mask
	...
	if (wt != wm || ht != hm) {
		puts("The mask && target image must have the same size.");
		abort();
	}
	if (cm != 1) {
	...
	if (cb != 3 || ct != 3) {
		puts("The background && target image must be colored.");
		abort();
	}
